...
- [x] **Step 6.: Write failing renderer test**
...
- [x] **Step 6.: Implement the renderer**
...
- [x] **Step 7.: Write failing CLI test**
...
- [x] **Step 7.: Run tests to verify failure**

Run: `go test ./cmd/sku/ -run TestEstimate -v`
Expected: FAIL — command not registered.

- [x] **Step 7.: Implement the command**
...
- [x] **Step 7.: Run CLI tests to verify pass**
...
- [x] **Step 8.: Exercise the command end-to-end**
...
- [x] **Step 8.: Add a CLAUDE.md quick path entry**
...
- [x] Every §4 "Estimate (three input forms) → inline" example parses under `ParseItem` (at least the `aws/ec2:m5.large:count=10:hours=730` form used in the spec; the LLM and S3 forms intentionally fail because they live in m5.2/m5.3).
- [x] `--config` and `--stdin` return exit-4 `flag_invalid` with a "deferred to m5.2" hint (covered in Task 7 test).
- [x] No new Go dependency appears in `go.mod` after `go mod tidy`.
- [x] No shard schema or pipeline file is touched.
- [x] All new files carry a clear package-level responsibility; nothing leaks business logic into `cmd/sku/`.
- [x] Commit messages follow the `feat(m5.1): …` / `docs(m5.1): …` pattern used by earlier milestones.

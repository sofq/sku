...
- [x] **Step 3: Note the current commit SHA for diffing later**
...
- [x] **Step 1: Write the seed SQL** — mirrors `seed_aws.sql` structure (full shard schema inline), then inserts a mix of rows. Keep prices strictly ordered so sort assertions don't need tolerance.
...
- [x] **Step 2: Sanity-check the fixture builds and opens**
...
- [x] **Step 3: Commit the fixture on its own**
...
- [x] **Step 1: Write the failing test first** — asserts signature and zero-behaviour (`Provider`+`Service` required).
...
- [x] **Step 3: Write the minimal `SearchFilter` + stub `Search`** to make the compile error go away and honor the validation only.
...
- [x] **Step 1: Write the failing test** — empty filter should return all 8 rows, sorted by `sku_id` for determinism.
...
- [x] **Step 3: Implement the base query** — borrow the scan path used by `lookupResource`, including `fillPrices`. Keep the query builder in one function that returns `(sql, args)` so later tasks can extend the predicate list without rewriting the scan path.
...
- [x] **Step 1: Write failing tests** — one assertion per filter.
...
- [x] **Step 3: Extend `buildSearchQuery`** with the three predicates, immediately after the provider/service clause:
...
- [x] **Step 3: Extend the builder** — both predicates filter on `resource_attrs`; a LEFT JOIN yields NULL when absent, so the predicate `ra.col >= ?` also (correctly) excludes NULL rows. That's exactly what the third test asserts.
...
- [x] **Step 3: Extend `buildSearchQuery`** with a correlated subquery on `prices`. Avoid a window function (pure-Go SQLite supports them but a `MIN(amount)` scalar subquery keeps the plan simple and uses no extra index).
...
- [x] **Step 4: Run; expect pass across the whole Search suite**
...
- [x] **Step 3: Implement** — `Sort` is allow-listed (never user-interpolated) and `Limit` binds as `?`. Replace the `ORDER BY ...` suffix in `buildSearchQuery` with an allow-list map, and surface the validation error before executing the query.
...
- [x] **Step 1: Create the command file with only the flag binding + guard checks**, no run body yet. This lets us wire into `root` and confirm `sku search --help` renders correctly before driving a catalog.
...
- [x] **Step 2: Register the command** — in `cmd/sku/root.go`, add `root.AddCommand(newSearchCmd())` alongside the existing `AddCommand` lines (order after `newSchemaCmd`).

- [x] **Step 3: Verify the help renders**
...
- [x] **Step 4: Verify the guard branches**
...
- [x] **Step 1: Flesh out `runSearch`** to open the shard at `catalog.ShardPath("<provider>-<service>")`, apply the stale gate, call `catalog.Search`, and render via `renderRows`. The pattern mirrors `runAWSCompute` in `aws_ec2.go`.
...
- [x] **Step 2: Verify the binary still builds**
...
- [x] **Step 1: Write the test helper** — mirrors the shape of `testutil_aws_m3a2_test.go`, building the seeded shard into `SKU_DATA_DIR/aws-ec2.db` via `catalog.BuildFromSQL` + `t.Setenv`.
...
- [x] **Step 2: Write the Cobra-level tests.**
...
- [x] **Step 3: Run the search Cobra tests**
...
- [x] **Step 4: Run the full `cmd/sku` package to ensure no regressions**
...
- [x] **Step 1: Build an openrouter shard + an aws-ec2 fixture shard** via the existing Make targets (validates nothing new broke in the ingest side):
...
- [x] **Step 2: Run `sku search` against the real shard**
...
- [x] **Step 3: Run a no-match scenario to verify exit 3**
...
- [x] **Step 1: Update the milestone line.** Replace the current-milestone pointer so future sessions see `m4.1` as the last-closed plan and `m4.2-compare` as the open one.
...
- [x] **Step 2: Append `sku search` to the Quick path.** Add a new block just before the Global flags section, after the final `gcp cloud-sql list` line:
...
- [x] **Step 3: Integration tests (catalog layer)**
...
- [x] **Step 4: Pipeline tests unchanged**
...
- [x] **Step 5: Diff summary for the milestone commit range**
...
- [x] **Step 6: Close the plan** — create the milestone-close commit, pointing future sessions at the next unimplemented milestone.

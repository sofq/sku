"""Normalize Azure Service Bus Retail Prices into messaging.topic rows.

Azure Service Bus exposes two topic-relevant tiers:
...
def ingest(*, prices_path: Path) -> Iterable[dict[str, Any]]:
    ...
    for item in items:
        ...
        sku_id = item.get("skuId") or f"SBT-{sku_name.upper()[:4]}-{region}"
    ...
    for region, raw_prices in sorted(std_raw.items()):
        ...
        if len(sorted_prices) != len(_STD_TIER_BOUNDS):
            print(
                f"warn: azure_service_bus_topics region {region!r} expected "
...
def main() -> int:
    ap = argparse.ArgumentParser(prog="ingest.azure_service_bus_topics")
    ...
    with args.out.open("w") as fh:
        for row in ingest(prices_path=prices_path):
            fh.write(dumps(row) + "\n")
            n += 1
    print(f"ingest.azure_service_bus_topics: wrote {n} rows", file=sys.stderr)
    if n == 0:
        return 2
    return 0

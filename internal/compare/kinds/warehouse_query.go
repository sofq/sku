package kinds

import (
	"context"
	"fmt"
	"strings"

	"github.com/sofq/sku/internal/catalog"
)

// WarehouseQuerySpec captures the warehouse.query equivalence shape.
//
// Mode defaults to "on-demand". Pass Mode = "capacity" or "storage" to compare
// those pricing rows. Edition filters the edition field for capacity rows;
// StorageTier filters the storage_tier field for storage rows.
type WarehouseQuerySpec struct {
	Mode        string // default "on-demand"; also "capacity" | "storage"
	Edition     string // optional json_extract(extra,'$.edition') filter for capacity mode
	StorageTier string // optional json_extract(extra,'$.storage_tier') filter for storage mode
	MaxPrice    float64
	Regions     []string
}

const defaultWarehouseMode = "on-demand"

// QueryWarehouseQuery runs the warehouse.query equivalence query against a single shard.
func QueryWarehouseQuery(ctx context.Context, c *catalog.Catalog, spec WarehouseQuerySpec) ([]catalog.Row, error) {
	where := []string{
		"s.kind = 'warehouse.query'",
		"t.commitment = 'on_demand'",
	}
	var args []any

	mode := spec.Mode
	if mode == "" {
		mode = defaultWarehouseMode
	}
	where = append(where, "json_extract(ra.extra, '$.mode') = ?")
	args = append(args, mode)

	if spec.Edition != "" {
		where = append(where, "json_extract(ra.extra, '$.edition') = ?")
		args = append(args, spec.Edition)
	}
	if spec.StorageTier != "" {
		where = append(where, "json_extract(ra.extra, '$.storage_tier') = ?")
		args = append(args, spec.StorageTier)
	}
	if spec.MaxPrice > 0 {
		where = append(where, "mp.min_price IS NOT NULL AND mp.min_price <= ?")
		args = append(args, spec.MaxPrice)
	}
	if len(spec.Regions) > 0 {
		placeholders := strings.Repeat("?,", len(spec.Regions))
		placeholders = placeholders[:len(placeholders)-1]
		where = append(where, "s.region IN ("+placeholders+")")
		for _, r := range spec.Regions {
			args = append(args, r)
		}
	}

	query := `
SELECT s.sku_id, s.provider, s.service, s.kind, s.resource_name, s.region,
       s.region_normalized, s.terms_hash,
       t.commitment, t.tenancy, t.os, t.support_tier, t.upfront, t.payment_option,
       ra.vcpu, ra.memory_gb, ra.storage_gb, ra.gpu_count, ra.gpu_model,
       ra.architecture, ra.extra,
       COALESCE(mp.min_price, 0) AS min_price
FROM skus s
JOIN terms t ON t.sku_id = s.sku_id
LEFT JOIN resource_attrs ra ON ra.sku_id = s.sku_id
LEFT JOIN (
  SELECT sku_id, MIN(amount) AS min_price FROM prices GROUP BY sku_id
) mp ON mp.sku_id = s.sku_id
WHERE ` + strings.Join(where, " AND ") + `
ORDER BY COALESCE(mp.min_price, 1e308) ASC, s.provider, s.resource_name, s.sku_id` //nolint:gosec // G202: WHERE composed from literals + placeholders only

	rs, err := c.QueryContext(ctx, query, args...)
	if err != nil {
		return nil, fmt.Errorf("compare: warehouse_query query: %w", err)
	}
	defer func() { _ = rs.Close() }()
	var out []catalog.Row
	for rs.Next() {
		r, err := scanVMRow(rs)
		if err != nil {
			return nil, err
		}
		if err := c.FillPrices(ctx, &r); err != nil {
			return nil, err
		}
		out = append(out, r)
	}
	return out, rs.Err()
}

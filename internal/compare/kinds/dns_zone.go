...
// DNSZoneSpec captures the dns.zone equivalence shape.
//
// Mode filters by extra.mode ("public" | "private").
...
// QueryDNSZone runs the dns.zone equivalence query against a single shard and
// returns rows with prices populated.
// Term pin: commitment='on_demand'. Mode is optionally filtered via
// json_extract(extra, '$.mode').
func QueryDNSZone(ctx context.Context, c *catalog.Catalog, spec DNSZoneSpec) ([]catalog.Row, error) {
	where := []string{
		"s.kind = 'dns.zone'",
...
ORDER BY COALESCE(mp.min_price, 1e308) ASC, s.provider, s.resource_name, s.sku_id` //nolint:gosec // G202: WHERE composed from literals + placeholders only
	...
	if err != nil {
		return nil, fmt.Errorf("compare: dns_zone query: %w", err)
	}

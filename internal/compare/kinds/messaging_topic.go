...
// MessagingTopicSpec captures the messaging.topic equivalence shape.
//
// Mode filters by extra.mode
// ("publish-http" | "publish-sqs" | "publish-email" | "standard" | "premium").
// Leave empty to return all modes.
type MessagingTopicSpec struct {
	Mode     string
	MaxPrice float64
	Regions  []string
}

// QueryMessagingTopic runs the messaging.topic equivalence query against a
...
ORDER BY COALESCE(mp.min_price, 1e308) ASC, s.provider, s.resource_name, s.sku_id` //nolint:gosec // G202: WHERE composed from literals + placeholders only
	...
	if err != nil {
		return nil, fmt.Errorf("compare: messaging_topic query: %w", err)
	}

...
// DBNoSQLSpec captures the db.nosql equivalence shape.
//
// Engine filters by terms.tenancy ("dynamodb" | "cosmos-sql" |
// "cosmos-mongo" | "firestore-native"). Leave empty to return all engines.
// Mode filters by extra.mode ("provisioned" | "serverless" | "native").
// Leave empty to return all modes.
type DBNoSQLSpec struct {
	Engine   string
	Mode     string
	MaxPrice float64
	Regions  []string
...
// QueryDBNoSQL runs the db.nosql equivalence query against a single shard and
// returns rows with prices populated.
// Term pin: commitment='on_demand'. Engine maps to terms.tenancy; Mode is
// optionally filtered via json_extract(extra, '$.mode').
func QueryDBNoSQL(ctx context.Context, c *catalog.Catalog, spec DBNoSQLSpec) ([]catalog.Row, error) {
	where := []string{
		"s.kind = 'db.nosql'",
...
ORDER BY COALESCE(mp.min_price, 1e308) ASC, s.provider, s.resource_name, s.sku_id` //nolint:gosec // G202: WHERE composed from literals + placeholders only
	...
	if err != nil {
		return nil, fmt.Errorf("compare: db_nosql query: %w", err)
	}
